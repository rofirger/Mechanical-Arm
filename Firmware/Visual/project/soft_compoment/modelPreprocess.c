...
//=====================================================================
//函数名称：Model_Normalization
//函数返回：无
//参数说明: 输入 image_28 处理   返回float类型 28×28
//功能概要：数据归一化处理
//=====================================================================
void Model_Normalization(image_28 input,float image_normalized[1][28][28])
{
	...
	float max =(float)input[0][0];
	float min =(float)input[0][0];
	...
	for(i=0;i<28;i++)
	{
		for(j=0;j<28;j++)
		{
			image_normalized[0][i][j]=(float)input[i][j];
		}
	}
	...
	for(i=0;i<28;i++)
	{
		for(j=0;j<28;j++)
		{
			if(image_normalized[0][i][j]>max)
			{
				max=image_normalized[0][i][j];
			}
			if(image_normalized[0][i][j]<min)
			{
				min=image_normalized[0][i][j];
			}
		}
	}
	...
	for(i=0;i<28;i++)
	{
		...
		{
			//min-max 标准化
			image_normalized[0][i][j]=(image_normalized[0][i][j]-min)/(max-min);
...
//			*((uint8_t*)dst+dst_cols*i+j)=(uint8_t)((1-u)*(1-v)*(*((uint8_t*)src+src_cols*i1+j1))+
//			(1-u)*v*(*((uint8_t*)src+src_cols*i1+j2))+u*(1-v)*(*((uint8_t*)src+src_cols*i2+j1))+
//			u*v*(*((uint8_t*)src+src_cols*i2+j2)));
        }
